...
tr_te3 <- tr_te %>%
  ...
  summarise(edu_sex_credit = mean(AMT_CREDIT),
            ...
            edu_sex_NEW_EXT_SOURCES_var = var(NNEW_EXT_SOURCES_MEAN),
...
dtrain <- data.matrix(dtrain, new.data1$new.tr)
dval <- data.matrix(dval, new.data1$new.te)
dtest <- data.matrix(dtest, new.data2$new.te)
