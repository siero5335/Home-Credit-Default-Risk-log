...
## package loading
```{r setup, include=FALSE}
library(tidyverse); library(lightgbm); library(magrittr); library(moments); library(missRanger);
...
```{r, include=FALSE}
set.seed(71)
work <- missRanger(work, pmm.k = 3, num.trees = 100)
```

```{r}
set.seed(42)
tri2 <- caret::createDataPartition(y, p = 0.8, list = F) %>% c()
dtrain <-  catboost.load_pool(work[tri2, ], label = y[tri2])
dval <-  catboost.load_pool(work[-tri2, ], label = y[-tri2])
```
