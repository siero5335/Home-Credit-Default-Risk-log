...
fn <- funs(mean, var, skewness, min, max, sum, n_distinct, (na.rm = TRUE))
...
df_corr = cor(na.omit(na.aggregate(sum_bureau)))
hc = findCorrelation(df_corr, cutoff=0.8)
hc = sort(hc)
dt1_num3 = as.data.frame(sum_bureau)[,-c(hc)]
rm_col_hc <- setdiff(colnames(sum_bureau), colnames(dt1_num3))

sum_bureau <- as.data.frame(sum_bureau)[, !(colnames(sum_bureau) %in% rm_col_hc[-1])]
rm(df_corr, dt1_num3, hc, rm_col_hc); gc(); gc()
...
df_corr = cor(na.omit(na.aggregate(sum_prev)))
hc = findCorrelation(df_corr, cutoff=0.8)
hc = sort(hc)
dt1_num3 = as.data.frame(sum_prev)[,-c(hc)]
rm_col_hc <- setdiff(colnames(sum_prev), colnames(dt1_num3))
...
new.data1 <- knnExtract(xtr = dtrain2, ytr = as.factor(y[tri2]), xte = dval2, k = 30)
new.data2 <- knnExtract(xtr = dtrain2, ytr = as.factor(y[tri2]), xte = dtest2, k = 30)
rm(fita1, fita2, fita3, dtrain2, dtest2 , dval2); gc(); gc()
