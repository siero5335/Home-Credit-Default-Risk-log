...
fn2 <- funs(mean, var, sum,  (na.rm = TRUE))
...
tr_te1 <- tr_te %>%
  group_by_(.dots=c("age_bucket")) %>%
  summarise(age_credit = mean(AMT_CREDIT),
            age_INCOME_TOTAL = mean(AMT_INCOME_TOTAL),
            age_credit_per_person = mean(credit_per_person),
            age_credit_per_child = mean(credit_per_child),
            age_PAYMENT_RATE = mean(PAYMENT_RATE),
            age_NEW_EXT_SOURCES_MEAN = mean(NEW_EXT_SOURCES_MEAN),
            age_EXT_SOURCE_1_MEAN = mean(EXT_SOURCE_1),
            age_EXT_SOURCE_2_MEAN = mean(EXT_SOURCE_2),
            age_EXT_SOURCE_3_MEAN = mean(EXT_SOURCE_3),
            age_OWN_CAR_AGE = mean(credit_per_child),
            
            age_credit_var = var(AMT_CREDIT),
            age_INCOME_TOTAL_var = var(AMT_INCOME_TOTAL),
            age_credit_per_person_var = var(credit_per_person),
            age_credit_per_child_var = var(credit_per_child),
            age_PAYMENT_RATE_var = var(PAYMENT_RATE),
            age_NEW_EXT_SOURCES_var = var(NEW_EXT_SOURCES_MEAN),
            age_EXT_SOURCE_1_var = var(EXT_SOURCE_1),
            age_EXT_SOURCE_2_var = var(EXT_SOURCE_2),
            age_EXT_SOURCE_3_var = var(EXT_SOURCE_3),
            age_OWN_CAR_AGE_var = var(credit_per_child)
            
  )

tr_te <- left_join(tr_te , tr_te1, by= c("age_bucket"))
...
tr_te2 <- tr_te %>%
  ...
  summarise(age_sex_credit = mean(AMT_CREDIT),
            ...
            age_sex_credit_per_person_var = var(credit_per_person),
            age_sex_credit_per_child_var = var(credit_per_child),
            age_sex_PAYMENT_RATE_var = var(PAYMENT_RATE),
            age_sex_NEW_EXT_SOURCES_var = var(NEW_EXT_SOURCES_MEAN),
            age_sex_EXT_SOURCE_1_var = var(EXT_SOURCE_1),
            age_sex_EXT_SOURCE_2_var = var(EXT_SOURCE_2),
            age_sex_EXT_SOURCE_3_var = var(EXT_SOURCE_3),
            age_sex_OWN_CAR_AGE_var = var(credit_per_child)
...
tr_te3 <- tr_te %>%
  ...
  summarise(edu_sex_credit = mean(AMT_CREDIT),
            ...
            edu_sex_credit_var = var(AMT_CREDIT),
            edu_sex_INCOME_TOTAL_var = var(AMT_INCOME_TOTAL),
            edu_sex_credit_per_person_var = var(credit_per_person),
            edu_sex_credit_per_child_var= var(credit_per_child),
            edu_sex_PAYMENT_RATE_var = var(PAYMENT_RATE),
            edu_sex_NEW_EXT_SOURCES_var = var(NEW_EXT_SOURCES_MEAN),
            edu_sex_EXT_SOURCE_1_var = var(EXT_SOURCE_1),
            edu_sex_EXT_SOURCE_2_var = var(EXT_SOURCE_2),
            edu_sex_EXT_SOURCE_3_var = var(EXT_SOURCE_3),
            edu_sex_OWN_CAR_AGE = var(credit_per_child)
  )
...
tr_te4 <- tr_te %>%
  ...
  summarise(org_sex_credit = mean(AMT_CREDIT),
            ...
            org_sex_credit_var = var(AMT_CREDIT),
            org_sex_INCOME_TOTAL_var = var(AMT_INCOME_TOTAL),
            org_sex_credit_per_person_var = var(credit_per_person),
            org_sex_credit_per_child_var = var(credit_per_child),
            org_sex_PAYMENT_RAT_varE = var(PAYMENT_RATE),
            org_sex_NEW_EXT_SOURCES_var = var(NEW_EXT_SOURCES_MEAN),
            org_sex_EXT_SOURCE_1_var = var(EXT_SOURCE_1),
            org_sex_EXT_SOURCE_2_var = var(EXT_SOURCE_2),
            org_sex_EXT_SOURCE_3_var = var(EXT_SOURCE_3),
            org_sex_OWN_CAR_AGE_var = var(credit_per_child)
  )
...
tr_te5 <- tr_te %>%
  ...
  summarise(org_edu_credit = mean(AMT_CREDIT),
            ...
            org_edu_credit_var = var(AMT_CREDIT),
            org_edu_INCOME_TOTAL_var = var(AMT_INCOME_TOTAL),
            org_edu_credit_per_person_var = var(credit_per_person),
            org_edu_credit_per_child_var = var(credit_per_child),
            org_edu_PAYMENT_RATE_var = var(PAYMENT_RATE),
            org_edu_NEW_EXT_SOURCES_var = var(NEW_EXT_SOURCES_MEAN),
            org_edu_EXT_SOURCE_1_var = var(EXT_SOURCE_1),
            org_edu_EXT_SOURCE_2_var = var(EXT_SOURCE_2),
            org_edu_EXT_SOURCE_3_var = var(EXT_SOURCE_3),
            org_edu_OWN_CAR_AGE_var = var(credit_per_child)
  )
...
tr_te6 <- tr_te %>%
  ...
  summarise(occ_credit = mean(AMT_CREDIT),
            ...
            occ_credit_var = var(AMT_CREDIT),
            occ_INCOME_TOTAL_var = var(AMT_INCOME_TOTAL),
            occ_credit_per_person_var = var(credit_per_person),
            occ_credit_per_child_var = var(credit_per_child),
            occ_PAYMENT_RATE_var = var(PAYMENT_RATE),
            occ_NEW_EXT_SOURCES_var = var(NEW_EXT_SOURCES_MEAN),
            occ_EXT_SOURCE_1_var = var(EXT_SOURCE_1),
            occ_EXT_SOURCE_2_var = var(EXT_SOURCE_2),
            occ_EXT_SOURCE_3_var = var(EXT_SOURCE_3),
            occ_OWN_CAR_AGE_var = var(credit_per_child)
  )
...
rm(tr_te1, tr_te2, tr_te3, tr_te4, tr_te5, tr_te6);gc(); gc()
...
tr_te <- as.data.frame(tr_te)[, !(colnames(tr_te) %in% rm_col_hc)];gc(); gc()

rm(df_corr, dt1_num3, rm_col_hc, fn2, inc_by_org, encoder, docs, hc, live, nzv_cols);gc(); gc()
