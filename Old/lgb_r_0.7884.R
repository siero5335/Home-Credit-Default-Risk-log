...
tr_te2 <- tr_te %>%
  ...
  summarise(age_credit = mean(AMT_CREDIT),
            ...
            age_GOODS_PRICE = mean(AMT_GOODS_PRICE),
            age_credit_per_person = mean(AMT_credit_per_person),
            age_credit_per_child = mean(AMT_credit_per_child)
            )

tr_te <- left_join(tr_te , tr_te2, by="age_bucket")
...
new.data1 <- knnExtract(xtr = dtrain2, ytr = as.factor(y[tri2]), xte = dval2, k = 50)
new.data2 <- knnExtract(xtr = dtrain2, ytr = as.factor(y[tri2]), xte = dtest2, k = 50)
